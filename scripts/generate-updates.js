// Quick script to process client data and generate SQL updates
// This processes the data from the SQL query results

const clients = [
  // I'll process a batch at a time
];
...
// Process and generate updates
const updates = [];
for (const client of clients) {
  const name = extractNameFromNotes(client.notes);
  if (name) {
    const escapedName = name.replace(/'/g, "''");
    updates.push(`UPDATE clients SET name = '${escapedName}' WHERE id = '${client.id}';`);
  }
}
