  ...
  return (
    <div className="flex min-h-screen bg-[#F7F8F8]">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        {/* Header */}
        <header className="bg-white border-b border-[#E4E7E7] px-8 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-[#1E2025] capitalize">
                {navigationItems.find(item => item.id === currentPage)?.label || 'Dashboard'}
              </h2>
            </div>
            <div className="flex items-center gap-4">
              <button 
                className="px-4 py-2 text-[#555A60] hover:text-[#1E2025] transition-colors"
                aria-label="Search"
              >
                Search
              </button>
